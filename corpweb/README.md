...
Отдельная пара AmneziaWG-интерфейсов на нодах (`az_escape`, `vpn_escape`) с
усиленной обфускацией handshake: ненулевые `S1`/`S2`, кастомные `H1..H4`,
генерируются случайно на CP при первом включении и хранятся в таблице
`wg_obfuscation_params`. Применяется для клиентов, которым стандартный AWG-
handshake блокируется ТСПУ.

### Требования на ноде

- Пакет `amneziawg` (DKMS-сборка amneziawg-kernel-module + `amneziawg-tools`).
  Ставится автоматически `agent/install.sh` при первой регистрации ноды через
  официальный PPA `ppa:amnezia/ppa` (Launchpad). Для ручной доустановки на
  уже существующую ноду — выполнить ту же функцию `install_amneziawg` из
  `agent/install.sh`.
- Systemd-юниты `awg-quick@az_escape.service` и `awg-quick@vpn_escape.service`
  создаются автоматически шаблоном amneziawg-tools.
...
(Обычные `antizapret` / `vpn` ифейсы продолжают жить в `/etc/wireguard/` и
использовать ванильный `wg-quick`.)

### Порты

- `500/udp` — `vpn_escape` (мимикрия под IKE/IPsec)
- `53443/udp` — `az_escape`

На CP в iptables DNAT для этих портов поднимается только при включённом
`ESCAPE_ENABLED`. Сами ифейсы на нодах поднимаются всегда, тумблер управляет
лишь доступностью извне.

### Админ-сторона (CP)

1. Панель → **Настройки AntiZapret** → секция «Обход блокировки».
2. Тумблер **ESCAPE_ENABLED** — включает/выключает DNAT на CP для портов
   500/53443.
3. Кнопка **«Перегенерировать параметры обфускации»** (красная) — пересоздаёт
   `S1/S2/H1..H4` в БД и re-render серверных конфигов. Все текущие escape-
   клиенты теряют связь до пересоздания ими конфига. Перед применением
   показывается confirm-диалог.

### Пользовательская сторона (ЛК)

- Над карточками конфигов появляется блок **«Опции скачивания»** с двумя
  toggle-карточками: «Резервный порт» (540/580) и «Обход блокировки»
  (500/53443). Карточки взаимоисключающие: активный «Обход блокировки»
  блокирует toggle «Резервный порт» и наоборот.
- Выбор применяется к последующим «Скачать» и «QR»; ранее выданные конфиги
  не меняются — клиент повторно скачивает, если хочет сменить режим.
- Файлы bypass-конфигов получают суффикс `B` в имени:
  - обычный: `<server>-az.zip` / `<server>-vpn.zip`
  - bypass: `<server>-azB.zip` / `<server>-vpnB.zip`

  Разные имена нужны потому, что Android (файловый менеджер и AmneziaWG-
  клиент) кеширует содержимое и не всегда перезаписывает туннель с тем же
  именем. Пользователь видит в клиенте оба туннеля и переключается одним
  тапом.

### QR

- QR большого antizapret-конфига (> 2953 байт, лимит QR v40) содержит
  короткоживущую download-link с JWT-токеном вместо самого conf'а.
  TTL токена — 3 минуты. Токен несёт флаги `bypass` и `backup`, чтобы
  public endpoint отдал именно тот conf, который был выбран в UI в момент
  генерации QR. Пересканирование старого QR после переключения тумблера
  не сработает — нужно закрыть модалку QR и открыть её заново.

### Ограничения

- Имя сетевого iface в Linux ограничено `IFNAMSIZ=15` байт. Поэтому short-
  iface имя `az_escape` (9 байт), а не `antizapret_escape`.
- AmneziaWG kernel-module сосуществует с ванильным `wireguard` kernel-module;
  они независимы, разные iface используют разные модули.
