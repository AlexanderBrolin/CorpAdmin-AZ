...
2. ✅ После того как `doall.sh` отработал на ноде (триггеры: admin-edit `config/*.txt` через Files Editor → SSE → hook=doall на ноде; cron `antizapret-update.timer`; ручной запуск админом; **с fix CorpAdmin-AZ-2yj — также admin-edit `*_INCLUDE` через UI настройки AZ → SSE → hook=doall_and_restart_antizapret на ноде**) — в течение 30 сек blob `antizapret:allowed_ips` обновлён до значения template на ноде, и скачиваемый через `download_config` `.conf` отражает новые настройки. Замечание: уже существующие на диске ноды клиентские файлы `/root/antizapret/client/wireguard/antizapret-client-*-am.conf` (generated при upstream `client.sh add`) сохраняют запечённый `${IPS}` и не обновляются — это upstream-ограничение, но оно не влияет на наш `download_config` (который рендерит .conf на лету из blob).
