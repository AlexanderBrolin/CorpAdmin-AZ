...
1. Клиент шлёт UDP handshake на `CP-IP:52443` (antizapret) или `CP-IP:52080` (vpn),
   или на backup ports `540` / `580`.
2. iptables PREROUTING (написанный `balancer.py`) DNAT'ит на одну из нод
   по вероятностному round-robin:
   `CP:52443 → нода:51443`, `CP:52080 → нода:51080`.
3. Нода принимает handshake на свой порт — tunnel установлен.
...
5. Escape-клиенты (`az_escape`, `vpn_escape`) подключаются **напрямую к ноде**
   на порты 53443 / 500 — без DNAT через CP.
...
| Трафик | Внешний порт (CP) | Куда летит | Балансировщик |
|---|---|---|---|
| HTTPS admin panel + API | 443 | nginx → backend :8000 | nginx (один CP) |
| WireGuard antizapret | 52443 / UDP | iptables DNAT → нода:51443 | balancer.py |
| WireGuard vpn | 52080 / UDP | iptables DNAT → нода:51080 | balancer.py |
| WireGuard backup antizapret | 540 / UDP | iptables DNAT → нода:51443 | balancer.py |
| WireGuard backup vpn | 580 / UDP | iptables DNAT → нода:51080 | balancer.py |
| AmneziaWG az_escape | 53443 / UDP | прямое подключение клиент → нода:53443 | не через CP |
| AmneziaWG vpn_escape | 500 / UDP | прямое подключение клиент → нода:500 | не через CP |

Escape-порты (500, 53443) добавляются в iptables только когда `escape_enabled=True`
в настройках CP
([`balancer.py:ESCAPE_PORTS`](../corpweb/backend/app/services/balancer.py#L23-L25)).
