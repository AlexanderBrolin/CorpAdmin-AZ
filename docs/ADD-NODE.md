...
### 10. `networking.service` падает из-за IPv6-секции провижининга

Установщик AntiZapret отключает IPv6 через `/etc/sysctl.d/99-disable-ipv6.conf`
(`net.ipv6.conf.{all,default,lo}.disable_ipv6=1`). Если провижининг хостера прописал в
`/etc/network/interfaces` отдельную секцию `iface <iface> inet6 static`, `ifup` не может
назначить IPv6-адрес и возвращает ошибку для **всего** интерфейса:

```
ifup[560]: Error: ipv6: IPv6 is disabled on this device.
ifup[498]: ifup: failed to bring up net0
systemd[1]: networking.service: Failed with result 'exit-code'.
```

IPv4 при этом назначается и связь есть — поэтому расхождение легко не заметить: нода
пингуется, ssh работает, установка идёт дальше. Опасность в том, что `networking.service`
остаётся `failed`, а `network-online.target` тянется именно им — и обе ключевые службы ноды
объявляют зависимость от этого target'а:

```bash
systemctl show antizapret         -p After -p Wants --value | tr ' ' '\n' | grep network
systemctl show corpweb-sync-agent -p After -p Wants --value | tr ' ' '\n' | grep network
# обе: network-online.target
systemctl list-dependencies network-online.target   # единственная зависимость — networking.service
```

Замечено 2026-09-18 при вводе wgfi5 (хостер Aeza, интерфейс `net0`). На нодах SolusVM
(wgfi4, `eth0`) этого нет — там `/etc/network/interfaces` сгенерирован без IPv6.

Диагностика и лечение:

```bash
systemctl list-units --failed                         # ожидаем пусто
journalctl -u networking -n 20 --no-pager | grep -i ipv6
cp /etc/network/interfaces /etc/network/interfaces.bak-$(date +%F)
# удалить из файла секцию 'auto <iface>' + 'iface <iface> inet6 static' целиком
systemctl restart networking
```

`restart` сразу после правки может ругнуться `Error: ipv4: Address already assigned` — это
артефакт: адрес уже на интерфейсе, а состояние `ifupdown` разошлось из-за предыдущего
падения. Единственная честная проверка — перезагрузка:

```bash
systemctl reboot
# после возврата:
systemctl is-failed networking     # ожидаем 'active', то есть НЕ failed
systemctl list-units --failed      # пусто
ip -4 -br a show <iface>           # адрес на месте
wg show interfaces; awg show interfaces   # все четыре iface поднялись
```

⚠️ В шапке файла Aeza пишет `# Generated by Aeza VM manager` и `# DO NOT EDIT DIRECTLY`,
то есть провижининг может вернуть секцию обратно при операциях с VM — ровно так же, как он
затирает `/etc/hosts` (раздел 1). После любого вмешательства хостера в машину проверять
заново.

### 11. Паритет с эталонной нодой — последний рубеж перед балансировщиком

Разделы 1–10 ловят по одной известной ошибке каждый. Этот — ловит любую, включая ещё
