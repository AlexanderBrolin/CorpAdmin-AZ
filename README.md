...
**Обход блокировки ТСПУ (опционально)**\
В CorpAdmin-AZ есть отдельный режим «Обход блокировки» — дополнительная пара AmneziaWG-интерфейсов с сильной обфускацией handshake (ненулевые `S1`/`S2`, кастомные `H1..H4`, генерируются случайно на установке). Используется, когда ТСПУ блокирует даже стандартный AWG-handshake. Порты: UDP `500` (vpn_escape, мимикрия под IKE) и UDP `53443` (az_escape). Требует на нодах пакет `amneziawg` — устанавливается автоматически скриптом `agent/install.sh` при первой регистрации ноды. Управляется админом из CP-панели (Настройки AntiZapret → секция «Обход блокировки»); клиент выбирает режим при скачивании конфига в ЛК.
